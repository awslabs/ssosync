...
func (s *syncGSuite) SyncUsers(query string) error {
	...
	for _, u := range deletedUsers {
		...
		if s.cfg.DryRun {
			...
			}).Info("IGNORED: deleting google user")
		} else {
			log.WithFields(log.Fields{
				"email": u.PrimaryEmail,
		...
		if err != aws.ErrUserNotFound && err != nil {
			log.WithFields(log.Fields{
				"email": u.PrimaryEmail,
			}).Warn("Error deleting google user, user not found")
			return err
		}
	...
	for _, g := range googleGroups {
		...
		for _, u := range s.users {
			...
			if _, ok := memberList[u.Username]; ok {
				if !*b {
					if s.cfg.DryRun {
						log.WithField("user", u.Username).Info("IGNORED: Adding user to group")
					} else {
				...
				if *b {
					if s.cfg.DryRun {
						log.WithField("user", u.Username).Info("IGNORED: Removing user from group")
					} else {
...
func (s *syncGSuite) SyncGroupsUsers(queryGroups string, queryUsers string) error {
	...
	for _, awsUser := range delAWSUsers {
		...
		if s.cfg.DryRun {
			log.Warn("IGNORED: deleting user")
		} else {
	...
	for _, awsUser := range updateAWSUsers {
		...
		if s.cfg.DryRun {
			log.Warn("IGNORED: updating user")
		} else {
	...
	for _, awsUser := range addAWSUsers {
		...
		if s.cfg.DryRun {
			log.Info("IGNORED: creating user")
		} else {
	...
	for _, awsGroup := range addAWSGroups {
		...
		var newAwsGroup *identitystore.CreateGroupOutput

		if s.cfg.DryRun {
			log.Warn("IGNORED: creating group")
		} else {
		...
		for _, googleUser := range googleGroupsUsers[awsGroup.DisplayName] {
			...
			if s.cfg.DryRun {
				log.WithField("user", awsUserFull.Username).Info("IGNORED: adding user to group")
			} else {
	...
	for _, awsGroup := range equalAWSGroups {
		...
		for _, awsUser := range deleteUsersFromGroup[awsGroup.DisplayName] {
			if s.cfg.DryRun {
				log.WithField("user", awsUser.Username).Warn("IGNORED: removing user from group")
			} else {
	...
	for _, awsGroup := range delAWSGroups {
		...
		if s.cfg.DryRun {
			log.Warn("IGNORED: deleting group")
		} else {
